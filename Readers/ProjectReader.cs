...
namespace Correct_test1.Readers
{
    public class ProjectReader
    {
        ...
        public List<string> ReadProjects(
            ...
            Editor ed)
        {
            return ReadProjectsInternal(db, ed);
        }

        // 新接口 给批量使用
        public List<string> ReadProjects(Database db)
        {
            return ReadProjectsInternal(db, null);
        }

        public List<ProjectNumberLocation> ReadProjectLocations(Database db)
        {
            ...
            using (Transaction trans = db.TransactionManager.StartTransaction())
            {
                ...
                foreach (ObjectId spaceId in spaces)
                {
                    ...
                    foreach (ObjectId id in btr)
                    {
                        ...
                        AddProjectLocation(entity, trans, locations, Matrix3d.Identity);
                    }
                }
            ...
            Matrix3d transform)
        {
            ...
            else if (entity is BlockReference block)
            {
                ...
                Matrix3d blockTransform = transform * block.BlockTransform;
                foreach (ObjectId id in btr)
                {
                    AddProjectLocation(
            ...
            List<ProjectNumberLocation> locations)
        {
            ...
            string projectNumber = GetProjectNumber(text);
            ...
            Editor ed)
        {
            ...
            using (Transaction trans = db.TransactionManager.StartTransaction())
            {
                ...
                foreach (ObjectId spaceId in spaces)
                {
                    BlockTableRecord btr = trans.GetObject(spaceId, OpenMode.ForRead) as BlockTableRecord;
                    ...
                    foreach (ObjectId id in btr)
                    {
                        ...
                        {
                            ReadBlock(block, trans, projects, ed);
                        }
                        else if (ent is DBText text)
                        {
                            AddProject(text.TextString, projects, ed);
                        }
                        else if (ent is MText mtext)
                        {
                            AddProject(mtext.Text, projects, ed);
                        }
                    }
                }
        ...
        private bool IsProjectNumber(string text)
        {
            ...
            string pattern = @"N\d{4}[A-Z]{2}\d{3}(-[A-Z0-9]+)?";
            return Regex.IsMatch(text, pattern);
        }

        private string GetProjectNumber(string text)
        {
            Match match = Regex.Match(text.ToUpper(), @"N\d{4}[A-Z]{2}\d{3}");
            if (match.Success)
                return match.Value;
            return null;
        }
